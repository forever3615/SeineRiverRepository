...
#              'qj':'1169691'}
    ...
    for key in yyh_id:
        print('正在获取 %3s 集资项目ID...' % chinese_name[key])
        team_x.update({key : get_member_proj_id(yyh_base_url + yyh_id[key])})
    
    print()
    
    for key in team_x:
        for item in team_x[key]:
    ...
    for modian in MODIAN_ARRAY:
        ...
        if r is False:
            ...
            cd = path + '/team_x'
            out_to_json(cd, r, str(modian.pro_id) + '_' + modian.title)
